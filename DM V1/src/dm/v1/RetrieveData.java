...
public class RetrieveData {
    ...
    public static ArrayList<String> RetrieveData(String SQLValue) {
        ...
        while (iterator.hasNext()) {
            ...
            CurrentArrayList.add(prettyPrint(ce.getProperties(), 0));       //Adding data retrieved from the iterator to the array
        }
    ...
    //Method that filters the data so that only the desired information is retrieved     
        ...
        for (ClientProperty entry : properties) {
            ...
            if (entry.getName().equals("Title") || entry.getName().equals("Code") || entry.getName().equals("IndicatorName") || entry.getName().equals("IndicatorCode")) {
                ...
                if (value.isPrimitive()) {
                    ...
                    if (entry.getName().equals("Code") || entry.getName().equals("IndicatorCode")) {  //Apends the code or indicator code after the Title or indicator name using a / to serperate
                                                                                                                 // This is to make hashmap generation after the array is returned easier
                        b.append("/");
                    }
    ...
    private static void intend(StringBuilder builder, int intendLevel) {                //String builder used to seperate incoming data
        for (int i = 0; i < intendLevel; i++) {
            builder.append("    ");
        }
    }

    private static ClientEntitySetIterator<ClientEntitySet, ClientEntity> readEntities(Edm edm, String serviceUri, String entitySetName) {
